...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        TestFairy.begin("88c8b54953e989691e1bd072ed8173a26c2f59bc")
        
        //Check access 
        //self.checkAccess(launchOptions)
        
        // Calling our local method to register for local notifications.
        self.registerForLocalNotifications(launchOptions)

        
        return true
    }
    func checkAccess(launchOptions: [NSObject: AnyObject]?){
        ...
        eventStore!.requestAccessToEntityType(EKEntityType.Reminder, completion: {
            ...
            if(error == nil){
                ...
                GlobalSettings.setCalendarAccessable(false)
    ...
    func registerForLocalNotifications(launchOptions: [NSObject: AnyObject]?) {
        ...
        reminderCategory.setActions([reminderActionConfirm, reminderActionSnooze], forContext: UIUserNotificationActionContext.Default)
        reminderCategory.setActions([reminderActionConfirm, reminderActionSnooze], forContext: UIUserNotificationActionContext.Minimal)
        
        // Register for notification: This will prompt for the user's consent to receive notifications from this app.
        let notificationSettings = UIUserNotificationSettings(forTypes: [.Alert, .Badge, .Sound], categories: [reminderCategory])
    ...
    func application(application: UIApplication, didReceiveLocalNotification notification: UILocalNotification) {
        ...
        GlobalSettings.showAlertView(notification.alertTitle!, alertMsg: notification.alertBody!, target: self.window!.rootViewController!)
    }
    ...
    func application(application: UIApplication, handleActionWithIdentifier identifier: String?, forLocalNotification notification: UILocalNotification, completionHandler: () -> Void) {
        ...
        if identifier == "ShowDetails" {
            ...
            notification.fireDate = NSDate().dateByAddingTimeInterval(60*1)
